...
namespace rage
{
	...
	public:
		...
		static std::string to_full_path(const char* file_name_c)
		{
			std::filesystem::path cwd = std::filesystem::current_path() / "onigiri/";

			std::string file_name(file_name_c);

			// things loaded via `onigiri:/` mount end up being /onigiri/onigiri, aka, wrong.
			// i.e. onigiri:/addons/nve resolves to {path}/onigiri/onigiri/addons/nve
			auto pos = file_name.find("onigiri:/");
